...
//	float distance;
	...
	//	bunsize_real = bunsize=(int*)realloc(bunsize_real,sizeof(int)*bunnrows); valgrind is giving segfault
		...
		while(i < bunnrows)
		{
			...
			if(buntime[i]<(tstart-1.0)) 
			{
				i++;
				continue;	
			}
			else if(buntime[i]>tstop)
			{
				break;
			}
			else
			//printf("bunch present\n");
			...
			else
			{
				...
				for(j=i+1;j<bunnrows;j++)
				{
					...
					if( ((buntime[j] - 20.0*(double)buntime_dfs[j]/1000000.0)-(buntime[i] + 20.0*(double)buntime_dsl[i]/1000000.0)) <= 20.0/1000000.0)
					{
						//printf("T\n");
						i=j;
